...
class AladdinTool:
    def __init__(self):
        # Base64 String with Padding Fix Logic
        ...
        # padding အလိုအလျောက်ဖြည့်ပေးတဲ့အပိုင်း (Base64 Error အတွက်)
        missing_padding = len(raw_url) % 4
        if missing_padding:
            raw_url += b'=' * (4 - missing_padding)
...
def setup_process():
    Logo()
    print(f"{y}[*] Detecting Network (Target: {DEFAULT_GATEWAY})...{w}")
    try:
        r = requests.get(f"http://{DEFAULT_GATEWAY}", timeout=5)
        # Regex search with safety check
        ...
        print(f"{r_clr}[!] Connection Failed: Could not reach {DEFAULT_GATEWAY}{w}")
        print(f"{r_clr}[!] Please ensure you are connected to Ruijie WiFi.{w}")
    time.sleep(2)
